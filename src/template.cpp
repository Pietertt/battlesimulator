...
Game* game = NULL;
Grid* grid = NULL;
KDTree* kdtree_blue = NULL;
KDTree* kdtree_red = NULL;
SDL_Window* window = 0;
...
int main(int argc, char** argv)
{
    ...
    game = new Game();
    grid = new Grid();
    kdtree_blue = new KDTree();
    kdtree_red = new KDTree();
    game->insert_grid(grid);
    game->insert_kdtree(kdtree_blue);
    game->insert_kdtree(kdtree_red);
    ...
    while (!exitapp)
    {
        ...
        if (pitch == (surface->get_width() * 4))
        {
            memcpy(target, surface->get_buffer(), SCRWIDTH * SCRHEIGHT * 4);
        }
        else
        {
            ...
            {
                memcpy(t, surface->get_buffer() + i * SCRWIDTH, SCRWIDTH * 4);
                t += pitch;
            }
        }
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            case SDL_KEYDOWN:
                ...
                break;
            case SDL_KEYUP:
                game->key_up(event.key.keysym.scancode);
                break;
            case SDL_MOUSEMOTION:
                game->mouse_move(event.motion.x, event.motion.y);
                break;
            case SDL_MOUSEBUTTONUP:
                game->mouse_up(event.button.button);
                break;
            case SDL_MOUSEBUTTONDOWN:
                game->mouse_down(event.button.button);
                break;
            default:
                break;
            }
